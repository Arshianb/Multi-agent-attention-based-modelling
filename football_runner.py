...
class FootballRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(self.episode_length):
                ...
                # values.shape = (self.n_rollout_threads, self.num_agents, 1) 
                # actions.shape = (self.n_rollout_threads, self.num_agents, 1) 
                # action_log_probs.shape = (self.n_rollout_threads, self.num_agents, 1)
                # rnn_states.shape = (self.n_rollout_threads, self.num_agents, 1, 64)
                # rnn_states_critic.shape = (self.n_rollout_threads, self.num_agents, 1, 64)
                obs, share_obs, rewards, dones, infos, available_actions = self.envs.step(actions)
                # obs.shape = (self.n_rollout_threads, self.num_agents, 217)
                # share_obs.shape = (self.n_rollout_threads, self.num_agents, 217)
                # rewards.shape = (self.n_rollout_threads, self.num_agents, 1)
                # dones.shape = (self.n_rollout_threads, self.num_agents)
                # infos.shape = len of self.n_rollout_threads and score_reward value of self.num_agents
                # available_actions.shape = (self.n_rollout_threads, self.num_agents, 19)
                dones_env = np.all(dones, axis=1)
                # dones_env.shape = (self.n_rollout_threads, ) boolian
                reward_env = np.mean(rewards, axis=1).flatten()
                # reward_env.shape = (self.n_rollout_threads, )
                train_episode_rewards += reward_env
                # train_episode_rewards.shape = (self.n_rollout_threads, )
                score_env = [t_info[0]["score_reward"] for t_info in infos]
                # score_env len of self.n_rollout_threads
    ...
    def collect(self, step):
        ...
        # self.buffer.share_obs[step].shape = self.n_rollout_threads, self.num_agents, obs_space
        # self.buffer.obs[step].shape = same as above
        # self.buffer.rnn_states[step].shape = self.n_rollout_threads, self.num_agents, 1, 64
        # self.buffer.rnn_states_critic[step].shape = same as above
        # self.buffer.masks[step].shape = self.n_rollout_threads, self.num_agents, 1
        # self.buffer.available_actions[step].shape = self.n_rollout_threads, self.num_agents, act_space
        # after np.concatenate it will be reshaped to self.n_rollout_threads*self.num_agents, ...
        ...
        # values, actions action_log_probs shape = self.n_rollout_threads, self.num_agents, 1
        # rnn_states.shape = self.n_rollout_threads, self.num_agents, 1, 64
    ...
    def insert(self, data):
        ...
        # dones.shape = self.n_rollout_threads, self.num_agents
        dones_env = np.all(dones, axis=1)
        # dones_env.shape = self.n_rollout_threads
        # make zero all the agents that had done their tasks.
    ...
    def eval(self, total_num_steps):
        ...
        while True:
            ...
            # until end of the evaluation eval_episodes
