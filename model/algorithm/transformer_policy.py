...
from model.utils.util import check
from model.algorithm.ma_transformer import MultiAgentTransformer
...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        # obs_space shape is diffrent relative to enviroment etc foorball has 217
        # act_space shape is diffrent relative to enviroment etc foorball has 19
        self.device = device
        ...
        if self.algorithm_name in ["mat", "mat_dec"]:
            from model.algorithm.ma_transformer import MultiAgentTransformer as MAT
        elif self.algorithm_name == "mat_gru":
            from model.algorithm.mat_gru import MultiAgentGRU as MAT
        elif self.algorithm_name == "mat_decoder":
            from model.algorithm.mat_decoder import MultiAgentDecoder as MAT
        elif self.algorithm_name == "mat_encoder":
            from model.algorithm.mat_encoder import MultiAgentEncoder as MAT
        else:
